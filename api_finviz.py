...
def scrapear(pags=range(1,3)):
    """
    :returns
    DataFrame con lista de acciones
    ticker, company, sector, industry, country, market cap, P/E, P/S , P/B, beta, avg volume, rel volume,
    price, volume, ipo date
    ...
    vista_simple = '111'
    vista_full = '152&ft=4&o=-ticker&c=0,1,2,3,4,5,6,48,64,65,67,70'
...
def ponerMinusculas(df):
    df = df.reset_index()
    df.columns = ['ticker',"company","sector","industry","country","market_cap",'beta',
                  'rel_volume', "price", 'volume', "ipo_date"]
    df = df.set_index("ticker")
    return df

def crearTablaFinviz(nombre="finviz",pags=range(1)):

    """ Descarga todas las acciones alguna informacion adicional y lo sube a la DB """

    finviz = scrapear(pags=pags)
    finviz = arreglarDatosFinviz(finviz)
    finviz = aNumerico(finviz, ['Market Cap'])
    finviz = aFlotante(finviz, ['Beta','Rel Volume','Price', 'Volume'])
...
if __name__ == '__main__':
    ...
    crearTablaFinviz(nombre='prueba')
